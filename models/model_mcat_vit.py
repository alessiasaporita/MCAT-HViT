...
###########################
### MCAT_ViT_Surv Implementation ###
###########################
class MCAT_ViT_Surv(nn.Module):
    def __init__(self, omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=2,
        ...
        super(MCAT_ViT_Surv, self).__init__()
    ...
    def forward(self, **kwargs):
        ...
        #MULTIMODAL TRANSFORMER
        h_omic_bag = torch.stack(h_omic).unsqueeze(0) #(1, 6, 256)
        h_path_coattn = h_path_coattn.transpose(0, 1) #(1, 6, 256)
        logits = self.transformer(h_path_coattn, h_omic_bag)
